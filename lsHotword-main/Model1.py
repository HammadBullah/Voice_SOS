...
HELP_DIR = r'E:\MInor\Model\lsHotword-main\lsHotword-main\Examples\data\positives'
NON_HELP_DIR = r'E:\MInor\Model\lsHotword-main\lsHotword-main\Examples\data\egatives'
NOISE_DIR = r'E:\MInor\Model\lsHotword-main\lsHotword-main\Examples\data\cckgrounds'
...
def load_and_convert_to_spectrograms(data_dir, label):
    ...
    for filename in os.listdir(data_dir):
        ...
        # Ensure all spectrograms have the same shape (e.g., resize to a common shape)
        Sxx_resized = resize_spectrogram(Sxx)  # Implement this function
...
def resize_spectrogram(spectrogram, target_shape=(129, 129)):
    # Calculate the difference between target shape and current shape
    height_diff = target_shape[0] - spectrogram.shape[0]
    width_diff = target_shape[1] - spectrogram.shape[1]
    
    # Pad the spectrogram to achieve the target shape
    padded_spectrogram = np.pad(spectrogram, ((0, max(0, height_diff)), (0, max(0, width_diff))),
                                mode='constant', constant_values=0)
    
    # Crop the spectrogram if necessary
    cropped_spectrogram = padded_spectrogram[:target_shape[0], :target_shape[1]]
    
    return cropped_spectrogram

# Load and convert audio files for each category
help_spectrograms, help_labels = load_and_convert_to_spectrograms(HELP_DIR, 0)  # Label for help class is 0
non_help_spectrograms, non_help_labels = load_and_convert_to_spectrograms(NON_HELP_DIR, 1)  # Label for non-help class is 1
noise_spectrograms, noise_labels = load_and_convert_to_spectrograms(NOISE_DIR, 2)  # Label for noise class is 2

# Combine spectrograms and labels for all categories
...
# Convert spectrograms to grayscale
...
def create_model(input_shape):
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(3, activation='softmax')  # Adjust this to the number of classes
    ])
    return model

# Create the model
input_shape = (129, 129, 1)  # Shape of spectrograms
...
# Train the model
history = model.fit(X_train_gray, y_train, epochs=30, validation_data=(X_val_gray, y_val))
...
# Evaluate the model on the help class
help_class_idx = 0
...
model.save(r'E:\MInor\Model\model.keras')
